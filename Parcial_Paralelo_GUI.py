...
def vecinos_vivos(matriz, fila, columna):
    ...
    vecinos = [(fila - 1, columna), (fila + 1, columna), (fila, columna - 1), (fila, columna + 1), (fila-1,columna-1), (fila-1, columna +1), (fila+1, columna-1), (fila+1, columna+1)] 
...
lock = th.Lock()  #se crea un lock, que es como un semaforo, para evitar la sobre escritura en una parte del código

def reglas(matriz, inicio, fin, nueva_matriz, historial_vivas):

    if contar_vivas(matriz) > 3: #las reglas se aplican unicamente cuando hay más de 3 celulas
        ...
        for i in range(inicio, fin):
            for j in range(len(matriz[0])):
                nueva_matriz[i][j] = matriz[i][j] #en caso de que hayan menos celulas, no se aplican las reglas y la matriz queda igual
        
        return nueva_matriz
    
def juego_paralelo(matriz, historial_vivas, numero_de_hilos):

    filas = len(matriz)
    nueva_matriz = crear_historial(matriz) 

    hilos = []
    rango = filas // numero_de_hilos #se divide para cada hilo 

    for i in range(numero_de_hilos):
        inicio = i * rango
        fin = filas if i == numero_de_hilos - 1 else (i + 1) * rango
        hilo = th.Thread(target=reglas, args=(matriz, inicio, fin, nueva_matriz, historial_vivas)) #se crean los hilos
        hilos.append(hilo) #se agregan los hilos a la lista
        hilo.start() #los hilos se empiezan a ejecutar

    for hilo in hilos: 
        hilo.join() #espera a que los hilos terminen y se juntan los resultados

    for i in range(filas):
        for j in range(len(matriz[0])):
            matriz[i][j] = nueva_matriz[i][j]

    with lock:
        agregar_celula_viva(matriz, historial_vivas) #se agregan las nuevas celulas, haciendo uso del lock para que no haya sobre escritura
    actualizar_historial(matriz, historial_vivas) 


def main_paralelo_gui():
    def ejecutar_juego():
        ...
        for gen in range(generacion):
            juego_paralelo(matriz, historial_vivas, num_hilos)
    ...
    def mostrar_matriz(matriz, titulo):
        ...
        for fila in matriz:
            texto_fila = ' '.join('O' if celula == 1 else '.' for celula in fila)
    ...
    ventana.title("Juego de la Vida de Conway (Paralelo)")
    ...
    tk.Label(frame_config, text="Número de hilos:").grid(row=2, column=0, sticky=tk.W, padx=5, pady=5)
    entry_hilos = tk.Entry(frame_config)
    entry_hilos.grid(row=2, column=1, padx=5, pady=5)

    #botón para ejecutar el juego
    tk.Button(frame_config, text="Ejecutar", command=ejecutar_juego).grid(row=3, column=1, columnspan=5, pady=10) 
